...
export default function PeriodTimeline({ data, max }: PeriodTimelineProps) {
  ...
  useEffect(() => {
    ...
    animate(".pt-dot", {
      ...
      duration: 380,
      // @ts-ignore
      delay: (_el: Element, i: number) => 500 + i * 28,
      easing: "outBack",
    } as any);
  }, [data, max]);
  ...
  return (
    <div className="space-y-2">
      <div className="overflow-x-auto">
        <svg viewBox={`0 0 ${W} ${H}`} className="min-w-[520px] w-full h-[160px]" role="img" aria-label="시대별 합격자 추이">
          <defs>
          ...
          {points.map((p) => (
            <circle key={p.d.label} cx={p.x} cy={p.y} r={p.d.value > 0 ? 1.7 : 1.1} fill={p.d.value > 0 ? "#0e4d7a" : "#9ca3af"} stroke="white" strokeWidth={0.6} className="pt-dot" opacity={0}>
              <title>{`${p.d.label} ${formatNumber(p.d.value)}명`}</title>
            </circle>
          ))}
          {points.map((p) => (
            <text key={`l-${p.d.label}`} x={p.x} y={H - 2} textAnchor="middle" fontSize="2.2" className="fill-ink-2">
              {p.d.label}
            </text>
          ))}
        </svg>
